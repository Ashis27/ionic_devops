...
import { App, NavController, NavParams, IonicPage, AlertController, ModalController, Events } from 'ionic-angular';
...
@IonicPage()
@Component({
  selector: 'page-dcbookingconfirmationafterpg',
  templateUrl: 'dcbookingconfirmationafterpg.html',
  providers: [LocalNotifications, CallNumber]
})
export class DCBookingConfirmationAfterPG {
  ...
  constructor( public events: Events,private callNumber: CallNumber, public localNotifications: LocalNotifications, public modalCtrl: ModalController, public appCtrl: App, public alertCtrl: AlertController, public navCtrl: NavController, public navParams: NavParams, public _dataContext: DataContext, private commonService: CommonServices) {
  ...
  getDiagnosticAppointmentDetailsFromCache() {
    this.commonService.getStoreDataFromCache(this.commonService.getCacheKeyUrl("diagnosticAppointmentdetails"))
      .then((result) => {
      ...
      .subscribe(response => {
        if (response.Status) {
          ...
          if (response.Result.PaymentStatus && (response.Result.PaymentResponse == "Success" || response.Result.PaymentResponse == "Pending")) {
            ...
            this.bookingConfirmationNotification();
            if (Math.round(Number(response.Result.ConsumerWalletRedemptionAmount)) > 0)
              this.walletTransactionNotification(response.Result);
            // }
            this.commonService.onMessageHandler("Your booking has been confirmed", 1);
            this.retrieveFeedbackQuestionList();
            // /this.removeItemFromCart(response.Result.GroupEntityID);
          }
  ...
  bookingConfirmationNotification() {
    this.localNotifications.schedule({
      id: new Date().getMilliseconds(),
      title: "Booking Confirmation!",
      text: "Booking diagnostic centre order for " + this.appoDetails[0].BookedForName + " has been confirmed " + " in " + this.centerDetails.CenterName,
      trigger: { at: new Date(new Date().getTime() + 100) },
      vibrate: true,
      //sound: "file://notification.mp3",
      icon: "file://logo.png",
    ...
    this.notificationDetails.Message = "Booking diagnostic centre order for " + this.appoDetails[0].BookedForName + " has been confirmed " + " in " + this.centerDetails.CenterName;
    // }
    this.notificationDetails.Contact = this.appoDetails[0].ContactNo;
    this.saveNotification(this.notificationDetails);
  }
  walletTransactionNotification(data) {
    this.localNotifications.schedule({
      id: new Date().getMilliseconds(),
      title: "Wallet Transaction!",
      text: data.ConsumerWalletRedemptionAmount + " HP wallet points have been debited from your wallet for the transaction #" + data.TransactionID + ".",
      trigger: { at: new Date(new Date().getTime() + 100) },
      vibrate: true,
      //sound: "file://notification.mp3",
      icon: "file://logo.png",
    ...
    this.notificationDetails.Message = data.ConsumerWalletRedemptionAmount + "HP wallet points have been debited from your wallet for the transaction #" + data.TransactionID + ".";
    // }
    this.notificationDetails.Contact = this.appoDetails[0].ContactNo;
    this.saveNotification(this.notificationDetails);
  }

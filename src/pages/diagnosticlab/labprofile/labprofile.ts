...
export class LabProfile {
  ...
  constructor(private iab: InAppBrowser,public _dataContext: DataContext, private commonService: CommonServices, public navCtrl: NavController, public navParams: NavParams, public modalCtrl: ModalController, public elemRef: ElementRef, public alertCtrl: AlertController) {
    this.selectedLabProfile = this.navParams.get('selectedLabProfile');
    this.selectedDiagnosticCenter = this.navParams.get('selectedCenter');
    this.selectedLabProfile.LabProfileName = this.commonService.convert_case(this.selectedLabProfile.LabProfileName);
    this.selectedLabProfile.PriceAfterDiscount = Math.round(this.selectedLabProfile.PriceAfterDiscount);
    this.selectedLabProfile.GenericDiscount = Math.round(this.selectedLabProfile.GenericDiscount);
    this.selectedLabProfile.Price = Math.round(this.selectedLabProfile.Price);
  }
  ionViewDidEnter() {
    this.packageCount = 0;
  ...
  getLabProfile() {
    this._dataContext.getLabProfile(this.selectedLabProfile.LabProfileId, this.selectedLabProfile.GroupEntityId)
      .subscribe(response => {
        if (response != null) {
          this.labTestList = response.LabTests;
          this.labProfileDetails = response;
        }
        else {
          this.commonService.onMessageHandler("Lab profile not available.", 0);
        }
      },
        error => {

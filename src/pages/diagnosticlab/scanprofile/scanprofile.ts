...
@IonicPage()
@Component({
  selector: 'page-scanprofile',
  templateUrl: 'scanprofile.html',
  providers: [InAppBrowser]
})

export class ScanProfile {
  selectedRadiologyScan: any;
  gender: string = "t1";
  addedPackage: any = {
  ...
  constructor(private iab: InAppBrowser,public _dataContext: DataContext, private commonService: CommonServices, public navCtrl: NavController, public navParams: NavParams, public modalCtrl: ModalController, public elemRef: ElementRef, public alertCtrl: AlertController) {
    this.selectedRadiologyScan = this.navParams.get('selectedScan');
    this.selectedDiagnosticCenter = this.navParams.get('selectedCenter');
    this.selectedRadiologyScan.LabTestName = this.commonService.convert_case(this.selectedRadiologyScan.LabTestName);
    this.selectedRadiologyScan.PriceAfterDiscount = Math.round(this.selectedRadiologyScan.PriceAfterDiscount);
    this.selectedRadiologyScan.GenericDiscount = Math.round(this.selectedRadiologyScan.GenericDiscount);
    this.selectedRadiologyScan.Price = Math.round(this.selectedRadiologyScan.Price);
  }
  ionViewDidEnter() {
    this.packageCount = 0;
  ...
  getLabTestProfile() {
    this._dataContext.GetLabTestProfile(this.selectedRadiologyScan.LabTestId, this.selectedRadiologyScan.GroupEntityId)
      .subscribe(response => {
        console.log(response);
        this.labScanProfileDetails = response;
      },
        error => {

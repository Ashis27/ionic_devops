...
@IonicPage()
@Component({
  selector: 'page-seeallscan',
  templateUrl: 'seeallscan.html',
  providers: [CallNumber,InAppBrowser]
})
export class SeeAllScan {
  ...
  //Get all the lab scan based on searched keyword
  getAllLabScan() {
    this.searchedResult = [];
    let query: any = {}
                      ...
                      this._dataContext.GetLabScans(query)
                        .subscribe(response => {
                          if (response.hits.total > 0) {
                            ...
                            this.labScanList = this.labScanList.concat(response.hits.hits);
                            ...
                            this.page = this.labScanList.length;
